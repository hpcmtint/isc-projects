...
func getStateFromCA(ctx context.Context, agents agentcomm.ConnectedAgents, dbApp *dbmodel.App, daemonsMap map[string]*dbmodel.Daemon, daemonsErrors map[string]string) (agentcomm.KeaDaemons, agentcomm.KeaDaemons, error) {
	...
	if len(caConfigGetResp) == 0 || caConfigGetResp[0].Result != 0 || caConfigGetResp[0].Arguments == nil || caConfigGetResp[0].Arguments.ControlAgent == nil || caConfigGetResp[0].Arguments.ControlAgent.ControlSockets == nil {
		dmn.Active = false
		errStr := "problem with config-get response from CA: "
		if len(caConfigGetResp) == 0 || caConfigGetResp[0].Arguments == nil || caConfigGetResp[0].Arguments.ControlAgent == nil || caConfigGetResp[0].Arguments.ControlAgent.ControlSockets == nil {
